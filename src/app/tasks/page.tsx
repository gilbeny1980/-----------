...
export default async function TasksPage({
  ...
  const [tasks, electricians, projects, counts] = await Promise.all([
    ...
    <div className="mx-auto max-w-5xl w-full px-4 py-6 space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-2 rounded-lg bg-white p-3 shadow-sm border border-slate-200">
        ...
        <FilterSelect
          label="פרויקט"
          paramName="projectId"
          current={params.projectId}
          options={[
            { value: "none", label: "ללא פרויקט" },
        ...
        <FilterSelect
          ...
          buildHref={buildHref}
        />
        {(params.status || params.priority || params.electricianId || params.projectId) && (
          <Link href="/tasks" className="text-sm text-slate-500 underline">
            נקה סינון
          </Link>
        )}
      </div>
            ...
            <tbody>
              {tasks.map((task) => (
                ...
                >
                  ...
                  <td className="p-3 text-slate-600">{task.project?.name ?? "—"}</td>
